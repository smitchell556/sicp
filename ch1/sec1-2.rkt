...
;;; of space and number of steps used by this process as the amount increases?
...
;; =~ 55 total nodes
;; The order of growth of space is linear in O(n) where n is the amount because
;; the stack depth of recursive calls (the stack) will only get as large as n.
;; The algorithm will travel as deep as it can (the amount), then unwind and
...
;; The order of growth in space and steps is logarithmic because the input is
;; repeatedly being cut down to 1/3 it's size.
...
(define (fast-prime? n k)
  (cond ((or (= n 2) (= n 3)) true)
	((or (<= n 2) (<= k 0) (= (remainder n 2) 0)) false)
	(else (miller-rabin-test n k))))
